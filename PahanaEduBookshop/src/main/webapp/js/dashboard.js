...
function loadDashboardStats() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#totalRevenue').text('Rs. ' + (data.totalRevenue || 0).toFixed(2));
        },
        error: function() {
            $('#totalBills').text('0');
...
function lookupItem(itemCode) {
    $.ajax({
        ...
        success: function(item) {
            ...
            showAlert('Item found: ' + item.title + ' (Stock: ' + item.stockQuantity + ', Price: Rs. ' + 
            ...
            const itemDiv = `
                <div class="d-flex justify-content-between align-items-center mb-2 p-2 border rounded">
                    <div>
                        ...
                        <small>Qty: ${billItem.quantity} × Rs. ${billItem.unitPrice.toFixed(2)}</small>
                    </div>
                    <div class="text-right">
                        <div class="font-weight-bold">Rs. ${billItem.totalPrice.toFixed(2)}</div>
...
function generateBill() {
    ...
    // Confirm bill generation with LKR currency
    if (!confirm(`Generate bill for ${currentCustomer.name} with total amount Rs. ${billTotal.toFixed(2)}?`)) {
        return;
    }
...
function generateBillPDF(billResponse) {
    ...
    // Create professional HTML content for printing/PDF with Sri Lankan details
    const billContent = `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
            <div class="header">
                ...
                <p style="margin: 5px 0;">No. 123, Education Street, Colombo 07, Sri Lanka</p>
                <p style="margin: 5px 0;">Phone: +94 11 234 5678 | Mobile: +94 77 123 4567 | Email: info@pahanaedu.lk</p>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="width: 17.5%;">Unit Price (LKR)</th>
                        <th style="width: 17.5%;">Total Price (LKR)</th>
                    </tr>
                </thead>
                <tbody>
                    ${billItems.map(item => `
                ...
                <tfoot>
                    <tr class="total-row">
                        ...
                        <td class="text-right">Rs. ${billTotal.toFixed(2)}</td>
                    </tr>
                </tfoot>
            </table>
            ...
            <div class="footer">
                ...
                <p>For any queries, please contact us at info@pahanaedu.lk</p>
                <p>Registered in Sri Lanka | All prices are in Sri Lankan Rupees (LKR)</p>
